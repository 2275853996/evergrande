...
prompt_and_wait(Device* device, int status) {
    for (;;) {
        ...
        Device::BuiltinAction chosen_action = Device::REBOOT;
...
int main(int argc, char **argv) {
    ...
    if (update_package != NULL) {
        ...
        if (!is_battery_ok()) {
            ...
            ui->Print("install_package retry_count:%d\n",retry_count);
    ...
    //    I("to install rk_img : %s.", update_rkimage);
        ...
        if(erase_baseparamer() != 0){
            printf("do not erase baseparamer.\n");
        }
    ...
    if ((status != INSTALL_SUCCESS && status != INSTALL_SKIPPED && !sideload_auto_reboot) ||
        ...
        ui->Print("prompt_and_wait.\n");
        Device::BuiltinAction temp = prompt_and_wait(device, status);
        if (temp != Device::NO_ACTION) {
            after = temp;
            ui->Print("prompt_and_wait: after = temp\n");
        }
    }
